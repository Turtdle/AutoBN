...
class DiamondGrid:
    def __init__(self):
        ...
        self.base_center_x = (self.base_diamond['left'][0] + self.base_diamond['right'][0]) // 2  
        self.base_center_y = (self.base_diamond['top'][1] + self.base_diamond['bottom'][1]) // 2 
    ...
    def get_diamond_center(self, row, col):
        ...
        center_x = self.base_center_x + (col * self.horizontal_step) + (row * -self.horizontal_step)
        center_y = self.base_center_y + (col * self.vertical_step) + (row * self.vertical_step)
    ...
    def point_in_diamond(self, x, y, row, col):
        ...
        vertices_list = [vertices['top'], vertices['right'], vertices['bottom'], vertices['left']]
...
def detect_boars_in_diamond_grid(screenshot_path=None, debug=False):
    ...
    for col in range(6):  # 0 to 5 (6 total)
        ...
        if min_x < 0 or min_y < 0 or max_x >= img_array.shape[1] or max_y >= img_array.shape[0]:
            continue
        ...
        for y in range(max(0, min_y), min(img_array.shape[0], max_y)):
            for x in range(max(0, min_x), min(img_array.shape[1], max_x)):
                ...
                if grid.point_in_diamond(x, y, row, col):
                    ...
                    if (boar_color_lower[0] <= pixel[0] <= boar_color_upper[0] and
                        boar_color_lower[1] <= pixel[1] <= boar_color_upper[1] and 
                        boar_color_lower[2] <= pixel[2] <= boar_color_upper[2]):
                        boar_pixels += 1
        ...
        if boar_pixels > boar_threshold:
            ...
            if debug:
                print(f"Boar detected at grid ({row},{col}) with {boar_pixels} boar-colored pixels")
                print(f"  Diamond center: ({center_x},{center_y})")
    
    return boar_positions, grid

def visualize_diamond_grid(screenshot_path=None, output_path="diamond_grid_overlay.png"):
    """
...
def check_winbad(top_left_x=1049, top_left_y=784, bottom_right_x=1071, bottom_right_y=819, target_color=(165, 161, 149)):
...
def turn_loop():
    ...
    heavy_num = 0
    while True:
        heavy_select_funcs = [select_heavy_1, select_heavy_2, select_heavy_3, select_heavy_4, select_heavy_5]
        if check_turn():
